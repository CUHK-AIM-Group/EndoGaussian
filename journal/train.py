...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    if not viewpoint_stack:
        ...
        viewpoint_stack_interp = scene.getInterpCameras()
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        if stage == 'coarse':
            viewpoint_stack = viewpoint_stack_train
            idx = 0
        elif iteration < 2001 or viewpoint_stack_interp is None:
            viewpoint_stack = viewpoint_stack_train
            idx = randint(0, len(viewpoint_stack)-1)
        else:
            if deformed_probs is None:
                with torch.no_grad():
                    deform_list = []
                    for viewpoint_cam in viewpoint_stack_train:
                        deform_list.append(render(viewpoint_cam, gaussians, pipe, background, stage=stage)["means3D_deform"])
                    deform_list = torch.stack(deform_list, dim=0)
                    deformed_extent = (deform_list[1:] - deform_list[:-1]).abs().mean(dim=-1).mean(dim=-1)
                    deformed_probs = deformed_extent / ( deformed_extent.sum() + 1e-5)
                    deformed_probs = deformed_probs.cpu().numpy()
                    print(deformed_probs)
                    
            viewpoint_stack = viewpoint_stack_interp[1::2]
            idx = np.random.choice(len(viewpoint_stack), p=deformed_probs)
        ...
        rendered_depths.append(depth.unsqueeze(0))
        ...
        rendered_depths = torch.cat(rendered_depths,0)
        ...
        if (gt_depths is not None) and ((gt_depths>=1).sum() > 100) and ((rendered_depths>=1).sum() > 100):
            try:
                depth_masks = (gt_depths>=1)
                depth_masks_render = (rendered_depths>=1)
                depth_masks = torch.logical_and(depth_masks, depth_masks_render)
                depth_masks = torch.logical_and(depth_masks, masks).float()
                
                rendered_depths = torch.clamp(rendered_depths, min=1.0, max=300)
                gt_depths = torch.clamp(gt_depths, min=1.0, max=300)
                rendered_depths = 1.0 / rendered_depths
                gt_depths = 1.0 / gt_depths
                depth_loss = l1_loss(rendered_depths, gt_depths, depth_masks)
            except:
        ...
        loss = Ll1 + 0.01 * img_tvloss + 0.001 * depth_loss
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, stage):
    if tb_writer:
        tb_writer.add_scalar(f'l1_loss', Ll1.item(), iteration)
        tb_writer.add_scalar(f'total_loss', loss.item(), iteration)
