...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, no_fine, test_speed=False, reconstruct=False):
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool, test_speed: bool, reconstruct: bool):
    ...
    with torch.no_grad():
        ...
        if not skip_test:
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, dataset.no_fine, test_speed=test_speed, reconstruct=reconstruct)
        if not skip_video:
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), hyperparam.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.skip_video, args.test_speed, args.reconstruct)